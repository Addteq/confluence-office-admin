...
*/
function embedBootstrapFormBuilder(){
    var nativeConfluenceUserProfileFields = [
        "userparam-personal",
        "fullName",
        "email",
        "userparam-phone",
        ...
        "userparam-company",
        "userparam-position",
        "userparam-department",
        "userparam-location"
    ];
   function addingTooltip() {
        jQuery("#input div.component div.controls").children().not("p.help-block").tooltip({
            title: function() {
                return "This field can be dragged and dropped"
            }
        })
    }(function(e, t) {
        function n(e) {
    ...
    function() {
        ...
        return '<form class=\'form\'>\n  <div class=\'controls\'>\n    <% var compiled =  _.reduce(items, function(str, v, k){ %>\n      <% v["name"] = k; %>\n      <% return str + popoverTemplates[v["type"]](v); %>\n    <% }, "") %>\n    <%= compiled %>\n    <hr/>\n    <button id="save" class=\'btn aui-button\'>Save</button><button id="cancel" class=\'btn aui-button\'>Cancel</button>\n  </div>\n</form>\n'
        ...
        return '<!-- Label -->\n<div class="control-group">\n  <div class="controls" style="">                     \n    <label id="<%- id %>" name="<%- id %>" style="font-size: 150%; margin-left: 50px;"><%- label %></label>\n  </div>\n</div>\n'
        ...
        return '<!-- Appended checkbox -->\n<div class="control-group">\n  <label class="control-label" for="<%- id %>"><%- label %></label>\n  <div class="controls">\n      <input id="<%- id %>" name="<%- id %>" class="<%- inputsize %>" dateformat="<%- dateFormat %>" type="text" isDate=true  <% if(required) {%> required <% } %> />\n    <% if (helptext.length > 0) { %><p class="help-block"><%- helptext %></p><% } %>\n  </div>\n</div>\n'
    }), define("text!templates/snippet/appendedtext.html", [], function() {
        return '<!-- Appended Input-->\n<div class="control-group">\n  <label class="control-label" for="<%- id %>"><%- label %></label>\n  <div class="controls">\n      <input id="<%- id %>" name="<%- id %>" class="<%- inputsize %>" isUserPicker=true placeholder="<%- placeholder %>" type="text" <% if(required) {%> required <% } %> />\n  </div>\n</div>\n'
        ...
        return '<!-- Select Basic -->\n<div class="control-group">\n  <label class="control-label" for="<%- id %>"><%- label %></label>\n  <div class="controls">\n    <select id="<%- id %>" name="<%- id %>" class="<%- inputsize %>" <% if(required) {%> required <% } %>><% _.each(options, function(option) { %>\n      <option><%- option %></option><% }); %>\n    </select>\n  </div>\n</div>\n'
    }), define("text!templates/snippet/selectmultiple.html", [], function() {
        return '<!-- Select Multiple -->\n<div class="control-group">\n  <label class="control-label" for="<%- id %>"><%- label %></label>\n  <div class="controls">\n    <select id="<%- id %>" name="<%- id %>" class="<%- inputsize %>" multiple="multiple" <% if(required) {%> required <% } %>><% _.each(options, function(option) { %>\n      <option><%- option %></option><% }); %>\n    </select>\n  </div>\n</div>\n'
    }), define("text!templates/snippet/textarea.html", [], function() {
        return '<!-- Textarea -->\n<div class="control-group">\n  <label class="control-label" for ="<%- id %>"><%- label %></label>\n  <div class="controls">                     \n    <textarea id="<%- id %>" name="<%- id %>" <% if(required) {%> required <% } %> ><%- textarea %></textarea>\n  </div>\n</div>\n'
        ...
        return n.View.extend({
            ...
            render: function(e) {
                var n = this,
                    i = n.model.get("title");
                i == "Select Basic" ? i = "Dropdown" : i == "Prepended Text" ? i = "Header" : i == "Appended Text" ? i = "User Picker" : i == "Appended Checkbox" && (i = "Date");
                var s = t.template(r)({
                    title: i,
    ...
    }), define("views/temp-snippet", ["jquery", "views/snippet", "text!templates/app/temp.html", "helper/pubsub", "text!templates/app/renderform.html"], function(e, t, n, r, s) {
        return t.extend({
            initialize: function() {
            ...
            mouseUpHandler: function(e) {
                var field = jQuery(this.tempForm).find('.control-group');
                if(jQuery(document).find('#temporaryField').length == 0){
                            
                           jQuery("<div id='temporaryField' style='visibility:hidden'></div>").insertAfter('#admin-body-content');
                           jQuery('#temporaryField').html(field);
                }else{
                    jQuery('#temporaryField').html(field);
                }                                  
        ...
        return i.extend({
            ...
            giveUniqueId: function(e) {
                ...
                typeof this.counter[t] == "undefined" ? this.counter[t] = 0 : this.counter[t] += 1, e.setField("id", t), typeof e.get("fields")["id2"] != "undefined" && e.setField("id2", t + "2-" + this.counter[t])
            },
        ...
        return n.View.extend({
            ...
            render: function() {
                ...
                })), this.$el.appendTo("#build form"), this.delegateEvents(), handleRemovedFields()
            },           
            ...
            handleTempMove: function(t) {
                ...
                var n = 0;
                jQuery(".pagecontent.aui-page-panel").length > 0 && (n = jQuery(".pagecontent.aui-page-panel").position().top);
                var r = n;
                t.pageX >= this.$build.position().left && t.pageX < this.$build.width() + this.$build.position().left && t.pageY - r >= this.$build.position().top && t.pageY - r < this.$build.height() + this.$build.position().top ? e(this.getBottomAbove(t.pageY - r)).addClass("target") : e(".target").removeClass("target")
            },
            handleTempDrop: function(t, n, r) {
                var i = 0;       
                jQuery(".pagecontent.aui-page-panel").length > 0 && (i = jQuery(".pagecontent.aui-page-panel").position().top);
                var s = i;
                if (t.pageX >= this.$build.position().left && t.pageX < this.$build.width() + this.$build.position().left && t.pageY - s >= this.$build.position().top && t.pageY - s < this.$build.height() + this.$build.position().top) {
                    var r = e(".target").index();
                    /*
                     * Refer: PLUG-5285
                     * In some scenario fieldId was duplicate because we were generating & assigning unique Id on "Save Form" button click on Configure Form page.
                     * hence instead of that now we are generating uniqueId on Field drop event.
                     * Note: Unique-Id are in the form of type-timestamp e.g. textinput-12334343, textarea-435345435
                     */
                    var idEle = n.attributes.fields.id,
                            oldVal = idEle.value;
                    /*If Existing Id is not native Confluence UserProfile Field and it is not the form of type-timestamp e.g. textinput-12334343*/
                    if (nativeConfluenceUserProfileFields.indexOf(oldVal) == -1 && oldVal.match(/^[A-z]+-[0-9]+$/) === null) {
                        newVal = oldVal + "-" + jQuery.now();
                        while(jQuery("#"+newVal).length > 0){ //Check if there is any other field exist with same Id.
                            newVal = oldVal + "-" + jQuery.now();
                        }
                        idEle.value = newVal;
                    }
                    e(".target").removeClass("target"), this.collection.add(n, {
                        ...
                        var getId =jQuery("#temporaryField").find('.controls').children().first().attr('id');                     
                        if(jQuery('#removedFields form .control-group').find('.controls').children("#"+getId).length == 0 && fieldIds.indexOf(getId) >= 0){                            
                             addRemovedField(jQuery('#temporaryField').html());                         
                        }
                       e(".target").removeClass("target")
                }
            }
    ...
    }), define("custom", function() {}), define("app/app", ["jquery", "underscore", "backbone", "collections/snippets", "collections/my-form-snippets", "views/tab", "views/my-form", "text!data/input.json", "text!data/radio.json", "text!data/select.json", "text!data/buttons.json", "text!templates/app/render.html", "text!templates/app/about.html", "custom"], function(e, t, n, r, i, s, o, u, a, f, l, c, h) {
        return {
            initialize: function() {
                new s({
                    title: "Input",
                    collection: new r(JSON.parse(u))
                }), e("#components .tab-pane").first().addClass("active"), e("#formtabs li").first().addClass("active"), jQuery("form#components #prependedtext").parent().parent().prepend('<label for="prependedtext" id="labelDragNDrop">Header  </label>'), new o({
                    title: "Original",
                    collection: new i(JSON.parse(document.getElementById("qwerty").text))
                })
            }
        }
    }), require.config({
